...
- 앱 아이콘: 민트색 주간 사용량 게이지와 `C` 모양을 Finder와 응용 프로그램 목록에 표시합니다.
- 자동 갱신: 1분마다. 잠자기에서 깨어난 뒤에도 조회합니다.
- 표시를 클릭: 최신 사용량을 바로 조회합니다. 팝업 메뉴나 별도 버튼은 없습니다.
- 표시를 우클릭: `종료` 메뉴를 열어 앱을 종료합니다.
...
`build.sh`는 `Assets/CodexUsage.png`에서 macOS 아이콘 파일(`.icns`)을 생성해 현재 폴더의 상위에 `Codex Usage.app`을 만들고 로컬 서명합니다. 새 Mac에서 사용하려면 Codex CLI 설치와 로그인이 필요합니다. 사용자가 직접 만든 로컬 앱이며 App Store 배포나 공증은 하지 않았습니다.
